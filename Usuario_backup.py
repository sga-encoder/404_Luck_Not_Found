...
class Usuario:
    """
    Clase que representa un usuario del casino    Attributes:
        _id (str): Identificador único del usuario
        _nombre (str): Nombre del usuario
        _apellido (str): Apellido del usuario
        _correo (str): Correo electrónico del usuario
        _contraseña (str): Contraseña del usuario
    ...
    @classmethod
    async def crear_usuario(cls, nombre: str, apellido: str, correo: str, contraseña: str):
        """
        Inicializa un nuevo usuario y lo guarda automáticamente en Firestore
        ...
        Returns:
            Usuario: Nueva instancia de Usuario guardada en Firestore
        """
        from servidor.src.model.usuario.UsuarioServicio import UsuarioServicio
        
        id = ""  # Generate or assign an ID as needed
        total_apostado = 0.0
        historial = []
        saldo = 1000
        usuario = cls(id, nombre, apellido, correo, contraseña, saldo, total_apostado, historial)
        
        # Guardar automáticamente en Firestore
        servicio = UsuarioServicio()
        await servicio.agregar_usuario(usuario)
        
        return usuario
    
    @classmethod
    def crear_usuario_local(cls, nombre: str, apellido: str, saldo: float = 0.0):
        """
        Inicializa un nuevo usuario solo en memoria (sin guardar en base de datos)
        ...
        Returns:
            Usuario: Nueva instancia de Usuario (solo en memoria)
        """
        id = ""  # Generate or assign an ID as needed
        total_apostado = 0.0
        historial = []
    ...
    def set_correo(self, correo: str) -> None:
        ...
        if correo and re.match(patron_correo, correo):
            self._correo = correo
        else:
    ...
    async def agregar_historial(self, registro: dict) -> None:
        from .UsuarioServicio import UsuarioServicio        servicio = UsuarioServicio()
        """
        ...
        await servicio.agregar_historial(self._id, registro)    async def aumentar_saldo(self, monto: float) -> None:
    ...
    def __repr__(self):
        return (
            ...
            f"contraseña: {'*' * len(self._contraseña)}\n"
            f"saldo: {self._saldo}\n"
